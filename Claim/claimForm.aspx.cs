    ...
    public partial class claimForm : System.Web.UI.Page
    {
            ...
            if (Session["User"] != null)
            {
                if (month != "")
                {
                    sql = "SELECT * FROM tbl_claim c " +
                        ...
                        "JOIN tbl_status_detail sd ON sd.detail_claim_id = c.claim_id AND sd.detail_status_id = c.claim_status LEFT JOIN tbl_claim_auto_id ci ON c.claim_id = ci.claim_id " +
                        "WHERE c.claim_delete = '0' AND (tbl_cpoint.cpoint_id Like '%" + Session["UserCpoint"].ToString() + "%' AND ci.claim_auto_id LIKE '%" + txtSearchComNumber.Text + "%' " +
                ...
                else
                {
                    sql = "SELECT * FROM tbl_claim c JOIN tbl_cpoint ON claim_cpoint = cpoint_id JOIN tbl_status ON status_id = claim_status LEFT JOIN tbl_user ON username = claim_user_start_claim " +
                          " JOIN tbl_status_detail sd ON sd.detail_claim_id = c.claim_id AND sd.detail_status_id = c.claim_status LEFT JOIN tbl_claim_auto_id ci ON c.claim_id = ci.claim_id WHERE c.claim_delete = '0' " +
                          " AND (tbl_cpoint.cpoint_id Like '%" + txtSearchCpoint.SelectedValue + "%' AND ci.claim_auto_id LIKE '%" + txtSearchComNumber.Text + "%' " +
                          " AND claim_equipment LIKE '%" + txtSearchComTitle.Text + "%' AND claim_budget_year = '" + txtSearchYear.SelectedValue + "' " +
        ...
        void GetReport(string key, int report)
        {
            ...
            if (car2has == "1")
            {
                ...
                else
                {
                    ...
                    if (licensePlate2 != "" && licensePlate2 != "-")
                    {
                        ...
                        if (provinceplate2 != province)
                        {
                            ...
                            {
                                strNote += " ใช้เส้นทาง" + comeFrom + "มุ่งหน้า" + directionIn + " โดยมี" + nameDrive + " เลขที่บัตรประจำตัวประชาชน " + idcard + " ที่อยู่ " + address + (telDrive.Trim() != "" && telDrive.Trim() != "-" ? " หมายเลขโทรศัพท์ " + telDrive : "") + " เป็นผู้ขับขี่ ";
                            }
                        }
                        else
                        {
                            ...
                            {
                                strNote += " จังหวัด" + province + " ใช้เส้นทาง" + comeFrom + "มุ่งหน้า" + directionIn + " โดยมี" + nameDrive + " เลขที่บัตรประจำตัวประชาชน " + idcard + " ที่อยู่ " + address + (telDrive.Trim() != "" && telDrive.Trim() != "-" ? " หมายเลขโทรศัพท์ " + telDrive : "") + " เป็นผู้ขับขี่ ";
                            }
                        }
                ...
                strNote += " ทั้งนี้ ด่านฯ " + cpointName + (point != "" ? " " + point : "");
                // อุปกรณ์ ประกันโครงการฯ
                //if(cpointName == "")
                //{
                //    strNote += " ได้ประสานแจ้งนายช่างโครงการ(นายช่างยิ่งยศ มิ่งมณี) ซึ่งเป็นเจ้าของอุปกรณ์ที่ได้รับความเสียหาย และด่านฯแจ้งความร้องทุกข์ไว้ที่ " + inform + " ไว้เป็นหลักฐานแล้ว";
                //}
                //else
                //{
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (licensePlate2 != "" && licensePlate2 != "-")
                    {
                        ...
                        if (provinceplate2 != province)
                        {
                            ...
                            {
                                strNote += " ใช้เส้นทาง" + comeFrom + "มุ่งหน้า" + directionIn + " โดยมี" + nameDrive + " เลขที่บัตรประจำตัวประชาชน " + idcard + " ที่อยู่ " + address + (telDrive.Trim() != "" && telDrive.Trim() != "-" ? " หมายเลขโทรศัพท์ " + telDrive : "") + " เป็นผู้ขับขี่รถยนต์คันดังกล่าว";
                            }
                        }
                        else
                        {
                ...
                {
                    strNote += " ทั้งนี้ ด่านฯ " + cpointName + (point != "" ? " " + point : "") + " ได้ประสานแจ้งนายช่างโครงการ(นายช่างยิ่งยศ มิ่งมณี) ซึ่งเป็นเจ้าของอุปกรณ์ที่ได้รับความเสียหาย และด่านฯแจ้งความร้องทุกข์ไว้ที่ " + inform + " ไว้เป็นหลักฐานแล้ว";
                }
                else
                {
                    strNote += "  ทั้งนี้ ด่านฯ " + cpointName + (point != "" ? " " + point : "") + " ได้ดำเนินการแจ้งความร้องทุกข์ไว้ที่ " + inform + " ไว้เป็นหลักฐานแล้ว";
