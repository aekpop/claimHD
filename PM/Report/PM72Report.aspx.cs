...
{
    public partial class PM72Report : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                string sqlrpt = "SELECT pm_ref_no,pm_act_stime,pm_act_etime,cpoint_name,pm_detail_annex,"
                              ...
                              + " JOIN tbl_company ON c.pm_corporate = tbl_company.company_name WHERE e.cpoint_sup = '3' AND "
                ...
                if (Session["statReport"].ToString() == "1")
                {
                    ...
                    timeThai = " สาย 7-2  (" + ssdate + " - " + eedate + ")";
                    Session["SSD"] = null;
                    Session["EED"] = null;
                }
                else if (Session["statReport"].ToString() == "0")
                {
                    timeThai = " สาย 7-2  (ปีงบประมาณ " + Session["BudgetY"].ToString() + ")";
                    Session["BudgetY"] = null;
                }
                else if (Session["statReport"].ToString() == "2")
                {
                    timeThai = " สาย 7-2  ( เดือน " + Session["MonthReport"].ToString() + " พ.ศ." + Session["YearReport"].ToString() + " )";
                ...
                function.Close();
                this.CRT72.ReportSource = rrr;
                rrr.ExportToHttpResponse(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, Response, true, "PMReport72");
            }
        }
    }
}