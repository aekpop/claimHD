...
{
    public partial class PM9Report : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                string sqlrpt = "SELECT pm_ref_no,pm_act_stime,pm_act_etime,cpoint_name,pm_detail_annex,"
                              ...
                              + " JOIN tbl_company ON c.pm_corporate = tbl_company.company_name WHERE e.cpoint_sup = '1' AND "
                ...
                if (Session["statReport"].ToString() == "1")
                {
                    ...
                    timeThai = " สาย 9  ("+ssdate + " - " + eedate+")";
                    Session["SSD"] = null;
                    Session["EED"] = null;
                }
                else if (Session["statReport"].ToString() == "0")
                {
                    timeThai = " สาย 9  (ปีงบประมาณ " + Session["BudgetY"].ToString() + ")";
                    Session["BudgetY"] = null;
                }
                else if (Session["statReport"].ToString() == "2")
                {
                    timeThai = " สาย 9  ( เดือน "+Session["MonthReport"].ToString() + " พ.ศ." + Session["YearReport"].ToString()+ " )";
