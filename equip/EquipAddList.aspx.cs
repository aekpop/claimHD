        ...
        protected void LoadPaging()
        {
            ...
            else if (Session["NewEQPKtype"].ToString() == "old")
            {
                ...
                if (rs.Read())
                {
                    ...
                    string sqlAddedd = "Select * FROM tbl_neweq_list LEFT JOIN tbl_location ON tbl_neweq_list.list_locate = tbl_location.locate_id ";
                    sqlAddedd += " WHERE newEQ_idx = '" + Session["NewEQPK"].ToString() + "' Order by newlist_id ASC ";
                    ...
                    resulttt.Text = dt.Rows.Count.ToString() + " รายการ";
                    gridadded.DataBind();
                }
