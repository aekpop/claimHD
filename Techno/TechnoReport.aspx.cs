...
namespace ClaimProject.Techno
{
    public partial class TechnoReport : System.Web.UI.Page
    {
        ...
        void Result()
        {
            ...
            {
                mouconst = " AND claim_month = '" + ddlMonth.SelectedItem + "' ";
            }
            if (ddlselectReport.SelectedValue == "1" || ddlselectReport.SelectedValue == "2")
            {
                sql = "SELECT f.`cpoint_name`,COUNT(cpoint_name) as CpointAmount, e.`device_name` , COUNT(device_name) as deviceAmount , device_initials , COUNT(device_initials) as initials " +
                    " FROM tbl_claim c INNER JOIN tbl_device_damaged d ON c.claim_id = d.claim_id RIGHT JOIN tbl_device e ON d.`device_id` = e.`device_id` " +
                    " JOIN tbl_cpoint f ON f.`cpoint_id` = c.`claim_cpoint` " +
                    " WHERE claim_delete = '0' AND ( c.`claim_budget_year` = '" + ddlbudget.SelectedValue + "' " + mouconst + " ) " +
                    " GROUP BY claim_cpoint, d.device_id ORDER BY claim_cpoint ";
            }
