...
namespace ClaimProject.Config
{
    public class ClaimFunction
    {
        ...
        string strConnString = "Server=10.51.20.201;User Id=adminclaim; Password=admin25;charset=utf8; Database=db_claim; Pooling=false"; //main server
        ...
        public string GetDirector(string data)
        {
            try
            {
                string[] readText = File.ReadAllLines(HostingEnvironment.MapPath("/Config/") + "director.txt");
                string txt = "";
                foreach (string s in readText)
                {
                    txt = s;
        ...
        public void GetListEstimate(DropDownList list, int level)
        {
            string[] readText = File.ReadAllLines(HostingEnvironment.MapPath("/Config/") + "Estimate.txt");
            foreach (string s in readText)
            {
                if (level == 0)
                {
